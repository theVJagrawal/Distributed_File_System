...
public class Node_1 {
	...
	static String recieveFolder="node1_prime";
	static String recievedFolder="node3_prime";
	static String send="";
	static String bootstrap_server="10.70.51.123:9092";
    ...
    static ArrayList<String> nodesdata=new ArrayList<>(Arrays.asList(new String[] {"node1","node2","node3"}));
	   ...
	   //0-----commands
	   //1-----recieving data
	   //2 --- for distributing data over nodes can be done by any node
	   //3-----not decided
...
//  	rancheck.start();
    	...
    	Thread reciverdatafromml=new Thread(new Runnable() {
	                ...
	                int createdfiles=0;
	             	for(ConsumerRecord<String,byte[]> record:records) {
	             		...
	             		if(turn==0) {
		             		...
		             		try (FileOutputStream fos = new FileOutputStream(System.getProperty("user.dir")+"/cifar-10/node1/"+recieveFolder+"/"+record.key())) {
		             		   ...
		             		   if(metadata1.containsKey(record.key().substring(0,record.key().length()-4)))continue;
		             		    metadata1.put(record.key().substring(0,record.key().length()-4), System.getProperty("user.dir")+"/cifar-10/node1/"+recieveFolder+"/"+record.key());
		             		    filewriter1.append(""+record.key().substring(0,record.key().length()-4)+" "+System.getProperty("user.dir")+"/cifar-10/node1/"+recieveFolder+"/"+record.key()+"\n");
		             		    filewriter1.flush();
    	 ...
    	 Thread recieverr=new Thread(new Runnable() {
	              ...
	              while(true) {
	                ...
	                int createdfiles=0;
	             	for(ConsumerRecord<String,byte[]> record:records) {
	             		...
	             		byte[] file_data=record.value();
	             		try (FileOutputStream fos = new FileOutputStream(System.getProperty("user.dir")+"/cifar-10/node1/"+recievedFolder+"/"+record.key())) {
	             		    fos.write(file_data); // Write the byte array to the file
	             		    createdfiles++;
	             		   if(metadata2.containsKey(record.key().substring(0,record.key().length()-4)))continue;
	             		    metadata2.put(record.key().substring(0,record.key().length()-4), System.getProperty("user.dir")+"/cifar-10/node1/"+recievedFolder+"/"+record.key());
	             		   
	             		    filewriter2.append(""+record.key().substring(0,record.key().length()-4)+" "+System.getProperty("user.dir")+"/cifar-10/node1/"+recievedFolder+"/"+record.key()+"\n");
	             		    filewriter2.flush();
	             		} catch (IOException e) {
	             		    e.printStackTrace();
	             		    System.out.println(record.key()+"    "+Arrays.toString(file_data));
			...
			try {
			 ...
			 System.out.println("Node 1 is totally started..............");
    ...
    private static void Initializemetadata() throws IOException{
    	 ...
    	 File folder=new File(System.getProperty("user.dir")+"/cifar-10/node1/"+recieveFolder);
         folder.createNewFile();
         File folder1=new File(System.getProperty("user.dir")+"/cifar-10/node1/"+recievedFolder);
         folder1.createNewFile();
         
         metafile1=new File(System.getProperty("user.dir")+"/cifar-10/node1/"+recieveFolder+"/metafile1.txt");
         metafile1.createNewFile();
         metafile2=new File(System.getProperty("user.dir")+"/cifar-10/node1/"+recievedFolder+"/metafile1.txt");
    ...
    public static void sendermaindata_cloning(String node,ConsumerRecord<String,byte[]> record) {
        ...
        ProducerRecord<String,byte[]> datatobesent=new ProducerRecord<String,byte[]>(node,3,record.key(),record.value(),record.headers());
        final String ns=node;
        producerbytes.send(datatobesent,new Callback() {
        	...
        	while(howmuch>0) {
        		   ...
        		   alreadytook.add(index);
					...
					try {
        		        ...
        		        producerbytes.send(record,new Callback() {
        	...
        	String[] str=command.split(" ");
           	...
           	for(int z=0;z<nodesdata.size();z++) {
		   		...
		   		if(!nodes.equals(topic) && !ignore.contains(nodes) && alive[z+1]==1) {
		   			...
		   			}else if(z==1 && alive[3]==1) {
		   				sendcommand("node3","sendfault"+ncommand.substring(4,ncommand.length()));
		   			}else if(z==2 ) {

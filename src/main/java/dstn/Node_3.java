...
public class Node_3 implements Datapre{
	public static int add=0;
	public static  volatile Scanner scr;
	static String recieveFolder="node3_prime";
	static String recievedFolder="node2_prime";
	static String send="";
//	static String bootstrap_server="10.60.4.137:9092";
    static String topic ="node3";
...
//  	rancheck.start();
	                ...
	                int createdfiles=0;
	             	for(ConsumerRecord<String,byte[]> record:records) {
	             		...
	             		if(turn==0) {
		             		...
		             		try (FileOutputStream fos = new FileOutputStream(System.getProperty("user.dir")+"/cifar-10/node3/"+recieveFolder+"/"+record.key())) {
		             		   ...
		             		   if(metadata1.containsKey(record.key().substring(0,record.key().length()-4)))continue;
		             		    metadata1.put(record.key().substring(0,record.key().length()-4), System.getProperty("user.dir")+"/cifar-10/node3/"+recieveFolder+"/"+record.key());
		             		    filewriter1.append(""+record.key().substring(0,record.key().length()-4)+" "+System.getProperty("user.dir")+"/cifar-10/node3/"+recieveFolder+"/"+record.key()+"\n");
		             		    filewriter1.flush();
	              ...
	              while(true) {
	                ...
	                int createdfiles=0;
	             	for(ConsumerRecord<String,byte[]> record:records) {
	             		...
	             		byte[] file_data=record.value();
	             		try (FileOutputStream fos = new FileOutputStream(System.getProperty("user.dir")+"/cifar-10/node3/"+recievedFolder+"/"+record.key())) {
	             		    fos.write(file_data); // Write the byte array to the file
	             		    createdfiles++;
	             		   if(metadata2.containsKey(record.key().substring(0,record.key().length()-4)))continue;
	             		    metadata2.put(record.key().substring(0,record.key().length()-4), System.getProperty("user.dir")+"/cifar-10/node3/"+recievedFolder+"/"+record.key());
	             		   
	             		    filewriter2.append(""+record.key().substring(0,record.key().length()-4)+" "+System.getProperty("user.dir")+"/cifar-10/node3/"+recievedFolder+"/"+record.key()+"\n");
	             		    filewriter2.flush();
	             		} catch (IOException e) {
	             		    e.printStackTrace();
	             		    System.out.println(record.key()+"    "+Arrays.toString(file_data));
 ...
 public static void sendping(String ttopic,int partition) {
    ...
    private static void Initializemetadata() throws IOException{
    	 ...
    	 File folder=new File(System.getProperty("user.dir")+"/cifar-10/node3/"+recieveFolder);
         folder.createNewFile();
         File folder1=new File(System.getProperty("user.dir")+"/cifar-10/node3/"+recievedFolder);
         folder1.createNewFile();
         
         metafile1=new File(System.getProperty("user.dir")+"/cifar-10/node3/"+recieveFolder+"/metafile1.txt");
         metafile1.createNewFile();
         metafile2=new File(System.getProperty("user.dir")+"/cifar-10/node3/"+recievedFolder+"/metafile1.txt");
